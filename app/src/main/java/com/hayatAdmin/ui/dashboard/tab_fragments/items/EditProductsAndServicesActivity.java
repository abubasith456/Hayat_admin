...
public class EditProductsAndServicesActivity extends BaseActivity {
    ...
    String timeStamp;
    String productId, productName, productImage, productDetails, productCategory;
    ...
    void onLayoutAddClick() {
        try {
            if (validate(editTextProductName.getText().toString(), editTextProductNameDetails.getText().toString(), editTextProductCategory.getText().toString())) {
                showProgress();
    ...
    private void uploadDataWithImage() {
        try {
            ...
            addFieldInfo.put("itemId", "" + productId);
            addFieldInfo.put("itemImage", "" + downloadImageUri);
            ...
            addFieldInfo.put("itemCategory", "" + productCategory);
            addFieldInfo.put("userId", "" + firebaseAuth.getCurrentUser().getUid());
            DocumentReference databaseReference = firebaseFirestore.collection(productCategory).document(productId);
    ...
    private void uploadDataWithoutImage() {
        try {
            ...
            HashMap<String, Object> addFieldInfo = new HashMap<>();
            addFieldInfo.put("itemId", "" + productId);
            addFieldInfo.put("itemImage", "" + productImage);
            addFieldInfo.put("itemName", "" + editTextProductName.getText().toString());
            addFieldInfo.put("itemPrice", "" + editTextProductNameDetails.getText().toString());
            addFieldInfo.put("itemCategory", "" + productCategory);
            addFieldInfo.put("userId", "" + firebaseAuth.getCurrentUser().getUid());
    ...
    private boolean validate(String productName, String productDetails, String productCategory) {
        boolean valid = true;
        try {
            ...
            if (productCategory.isEmpty()){
                editTextProductCategory.setBackground(getResources().getDrawable(R.drawable.background_rounded_edit_text_error));
                mTextViewErrorProductCategory.setVisibility(View.VISIBLE);
                mTextViewErrorProductCategory.setText("PLease select category");
            }else {
                editTextProductCategory.setBackground(getResources().getDrawable(R.drawable.background_rounded_edit_text_gray));
                mTextViewErrorProductCategory.setVisibility(GONE);
            }
        } catch (Exception exception) {
            Log.e("Error ==> ", "" + exception);
        }
